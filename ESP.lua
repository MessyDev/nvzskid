...
local function GetUnitTeam(unit)
    local tag = unit:FindFirstChild("TEAM")
...
local function LocalTeam()
    ...
    local tag = char:FindFirstChild("TEAM")
...
local UtilityFunctions = {
	InitChecks = function(self, Entry)
		...
		Entry.Connections.UpdateChecks = Connect(__index(RunService, DeveloperSettings.UpdateMode), function()
            ...
            local theirTeam = GetUnitTeam(Player)
            Checks.Team = myTeam ~= theirTeam
          end
...
local LoadESP = function()
    ...
    if humanoid then
        UtilityFunctions:WrapObject(unit)
    end
  end

	local ServiceConnections = Environment.UtilityAssets.ServiceConnections

	ServiceConnections.UnitAdded = UnitsFolder.ChildAdded:Connect(function(unit)
...
ServiceConnections.UnitRemoved = UnitsFolder.ChildRemoved:Connect(function(unit)
    local entry = UtilityFunctions.GetObjectEntry(unit)
    if entry then
        UtilityFunctions.UnwrapObject(entry.Hash)
    end
end)
